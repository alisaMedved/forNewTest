...
class Posts extends Component {
  ...
  // запрос пошел по параметрам из state сразу при монтировании компонента
  componentDidMount() {
  ...
  // обработчик в инпуте поисковике
  ...
  // обработчик изменения количества отображаемых постов в селекте
  ...
  // при нажатии enter в инпуте - сразу обнуляет страницу
  // делает запрос
  getSearch = ({ key }) => {
    if (key === "Enter") {
  ...
  // функция юзаемая в обработке запроса
  setNews = (result) => {
    this.setState({ result });
  };

  // смена страницы на пагинации часть логики надо см в самом Paginstion
